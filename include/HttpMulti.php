...
class VideoCrawler {
    ...
    private function createHandle($task) {
        ...
        $options = [
            ...
            CURLOPT_HTTP_VERSION => defined('CURL_HTTP_VERSION_2_0') ? CURL_HTTP_VERSION_2_0 : 1, //设置 http 协议版本 1=CURL_HTTP_VERSION_1_1(兼容PHP5.6)
    ...
    private function generateHeaders($url) {
      ...
      private function generateCookies() {
        ...
        foreach ($this->commonCookies as $name) {
            $cookies[$name] = $this->generateRandomHex(8);
        }
    ...
    //生成随机十六进制字符串（兼容PHP5.6，无random_bytes时自动降级）
    private function generateRandomHex($bytes) {
    ...
    private function generateReferer($url) {
        ...
        $domain = isset($parsed['host']) ? $parsed['host'] : 'youku.com';
    ...
    private function processResponse($done) {
        ...
        $ch = $done['handle'];
        $task = isset($this->handles[(int) $ch]) ? $this->handles[(int) $ch] : null;
        if (!$task)
            return;
        ...
        $result = [
            ...
            'error' => $error,
            'url' => $task['url'],
            'proxy' => (isset($task['options']['proxy']) ? $task['options']['proxy'] : null),
            'referer' => $this->findRefererHeader(isset($info['request_header']) ? $info['request_header'] : '')
        ];

        if ($error || (isset($info['http_code']) ? $info['http_code'] : 0) >= 400) {
