...
class YUN {
    ...
    public static function parse($val, $type = PARSE_URL) {
        ...
        switch ($type) {
            ...
            case 3 : //	搜索资源 使用ID   		
                return self::getvideobyid($val["flag"], $val["id"], isset($val["part"]) ? $val["part"] : '1');
...
//根据url或视频名称取视频信息
    public static function getvideo($val, $lswrod = false) {
        ...
        foreach ($api as $key => $val) {
            ...
            $parm = (isset($val['type']) ? $val['type'] : ('0' === '1')) ? "/wd/" : "?wd=";
 ...
 //if  (DEBUG){ var_dump($info);}
       ...
       // if  (DEBUG){ var_dump($vodurl);}
        ...
        if (sizeof($info[0]['video']) >= $num) {
            ...
            $pv=isset($num) ? (int)$num - 1 : 0;
            $vod = isset($info[0]['video'][(int)$num-1]) ? $info[0]['video'][(int)$num-1] : $info[0]['video'][0];
            $vod = explode('$', $vod);
            //类型转换
            $type = $info[0]['flag'];
        ...
        if ($data) {
            ...
            if (substr(trim($data), 0,1)==='{') {
              ...
              return $videoinfo;
                ...
                $img = isset($xml->list->video->pic) ? (string) $xml->list->video->pic : '';
                $name = isset($xml->list->video->name) ? (string) $xml->list->video->name : '';

                foreach ($xml->list->video->dl->dd as $video) {
                    $flag = isset($video['flag']) ? (string) $video['flag'] : '';
        ...
        //json  
         ...
         //其他格式     
           ...
           for ($index = 2; $index <= $pagecount; $index++) {
                    ...
                    if (isset($api[$froms[$key]]['type']) ? $api[$froms[$key]]['type'] : '0') {
            ...
            //json格式处理
                if (strpos($html, '{') === 0) {
                    ...
                    foreach ($json->list as $video) {
                        ...
                        $type = isset($video->class["type_name"]) ? $video->class["type_name"] : '';
                        $flag_name = count($json->list) > 1 ? $flag : (isset($api[$froms[$i]]['name']) ? $api[$froms[$i]]['name'] : '');
    ...
    public static function getvid($data) {
     ...
     try {
       ...
       if($onely==="<"){
             ...
             return isset($obj->list->video[0]->id) ? (string) $obj->list->video[0]->id : "";
             
       }else{
            $obj = json_decode($data);
            return isset($obj->list[0]->vod_id) ? $obj->list[0]->vod_id : "";
            ...
            if (preg_match($val, $url)) {
                ...
                foreach ($value as $word) {
                    if (preg_match($word, $title, $matches)) {
                        ...
                        $num = isset($matches[2]) ? $matches[2] : 1;
                        break;
                    }
                }
                if ($name != "") {
