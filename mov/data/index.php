...
if (!empty($_GET['random'])) {
    ...
    if (empty($_REQUEST["act"])) {
        ...
        $retCache= data(array("act" => $_REQUEST["act"],"id" => $_REQUEST["id"],'cat'=>$_REQUEST["cat"],'site'=>$_REQUEST["site"],'size'=>(isset($_REQUEST["size"]) ? $_REQUEST["size"] : '')));
 ...
 function data($data){
     ...
     $act=isset($data['act']) ? $data['act'] : 'index';
     $retDate=array();
          ...
          //检查是否成功
           if(isset($retArray['msg']) && $retArray['msg']=='Success'){
             ...
             $d= $retArray['data'];
              ...
              $site=isset($data['site']) ? $data['site'] : $d['playlink_sites'][0]; 
          ...
          if(isset($retArray['msg']) && $retArray['msg']=='Success'){
           ...
           //取播放列表
            if($ismore){
              ...
              $site=isset($data['site']) ? $data['site'] : $d['playlink_sites'][0]; $retDate['site']=$site;
           ...
           //取播放页数据  
              ...
              /*    item      */
               $item=array(
                 ''.implode(' ', (isset($d['area']) ? $d['area'] : array())).' '.implode(' ', (isset($d['moviecategory']) ? $d['moviecategory'] : array())),
                 '导演:'.implode(' ', (isset($d['director']) ? $d['director'] : array())),
                 '演员:'.implode(' ', (isset($d['actor']) ? $d['actor'] : array())),
            ...
            $guess_list=data_get_list($parm);
            $guess=isset($guess_list['list']) ? $guess_list['list'] : array();
             ...
             if(isset($d['playlinks']))
              {
                  ...
                  foreach ($d['playlinks'] as $key => $url) {
                      $from_name=isset($data_site[$key]) ? $data_site[$key] : $key;
             ...
             $retDate=array(
                 ...
                 'id'=>(isset($d['ent_id']) ? $d['ent_id'] : ''),
                 'from'=>$from,
                 'item'=>$item,
                 'guess'=>$guess,
                 'pic'=>(isset($d['cdncover']) ? $d['cdncover'] : ''),
                 'sites'=>$d['playlink_sites'], 
                 'total'=>(isset($d['total']) ? $d['total'] : ''),    //剧集集数
                 'allupinfo'=>(isset($d['allupinfo']) ? $d['allupinfo'] : array()),    //剧集更新集数
                 'hasmore'=>$hasmore);
    ...
    // var_dump($data,$retArray);
     ...
     if(isset($retArray['errno']) && $retArray['errno']==0){
             foreach ($retArray['data']['movies'] as $list) {
                 ...
                 $score=isset($list['score'])?$list['score']:(isset($list['doubanscore'])?$list['doubanscore']:(isset($list['upinfo'])?$list['upinfo']:(isset($list['pubdate'])?$list['pubdate']:'')));
     
              $dlist=array(
                  'title'=>$list['title'],
                  'id'=>$list['id'],
                  'pic'=>(isset($list['cdncover']) ? $list['cdncover'] : $list['cdncover']),
                  'hint'=>(isset($list['pv']) ? $list['pv'] : ''),
                  'desc'=>(isset($list['description']) ? $list['description'] : '暂无简介'),
                  'update'=>(isset($list['pubdate']) ? $list['pubdate'] : ''),
                  'upinfo'=>(isset($list['upinfo']) ? $list['upinfo'] : ''),
                  'tag'=>(isset($list['tag']) ? $list['tag'] : ''),        //综艺特有标签
                  'total'=>(isset($list['total']) ? $list['total'] : ''),    //电视剧/动漫特有标签
                  'score'=>$score, // 电影评分
                  'comment'=>(isset($list['comment']) ? $list['comment'] : '')
    ...
    //exp: https://api.so.360kan.com/index?force_v=1&kw=电影&pageno=1
     ...
     if(isset($retArray['code']) && $retArray['code']==0 ){
         ...
         foreach ( $rows as $list) {
              $dlist=array(
                  'title'=>$list['titleTxt'],
                  'id'=>(isset($list['en_id']) ? $list['en_id'] : getUrlid($list['url'])),
                  'pic'=>$list['cover'],
                  'score'=>(isset($list['score']) ? $list['score'] : ''),
                  'desc'=>(isset($list['description']) ? $list['description'] : '暂无简介'),
                  'year'=>(isset($list['year']) ? $list['year'] : ''),
                  'type'=>(isset($list['cat_name']) ? $list['cat_name'] : ''),
                  'upinfo'=>(isset($list['coverInfo']['txt']) ? $list['coverInfo']['txt'] : ''),
                  'cat'=>(isset($list['cat_id']) ? $list['cat_id'] : ''),
                  'comment'=>(isset($list['comment']) ? $list['comment'] : ''),
                  'tag'=>(isset($list['tag']) ? $list['tag'] : ''),
              );
